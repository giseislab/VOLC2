...
class catalog():
    ...
    def writeXML(self,filename,defaultIconStyle='2'):
        ...
        f.close()
        return
    def writeXMLSm(self,filename,defaultIconStyle='2'):
        '''
        method writes catalog to xml file specified by filename
        '''
        iconStyle=defaultIconStyle
        ...
        for i in range(len(self.data)):
            ...
            if e.coordinate:
                f.write('<param name="latitude" value="%.4f"/>'%e.coordinate.latitude)
                f.write('<param name="longitude" value="%.4f"/>'%e.coordinate.longitude)
            if e.depth:
                f.write('<param name="depth" value="%.2f"/>'%e.depth)
            if e.magnitude[0]:
                f.write('<param name="magnitude" value="%.2f"/>'%e.magnitude[0].magnitude)
