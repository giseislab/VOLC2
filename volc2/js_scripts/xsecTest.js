...
function getXsec(){
	...
	//var path = new Array ();
	//path.push(new google.maps.LatLng(loc1));
	//path.push(new google.maps.LatLng(loc2));
	//var pathrequest = {
	//	'path': path,
	//	'samples': samp
	//};
	//elevSvc.getElevationAlongPath( pathrequest, getElevation );
	//v2 solution
	getElevation();
	var inc = xsecLength/(samp-1); // get increment for each data point in profile
	for (var i = 0; i < samp; i++){
		kmdata = inc*i;
		edata = ekm[i];  //data in km
		profile.push([kmdata, edata]);
	}
...
function plotXsec( xsecLength){
	...
	f = Flotr.draw(
		$('chart_div'),[ 
			...
			{data:profile, label:'elev_profile', lines: {show: true, fill: false, lineWidth: 2}, points: {show: false}}
		],{
			title: 'Cross-Section',
			fontSize: 14,
			xaxis:{
...
//function getElevation(results,status) {
//	if (status ==google.maps.ElevationStatus.OK) {
//		elevation = results;
//		for (var i = 0; i < samp; i++){
//			ekm[i] = results[i].elevation/1000;
//		}
//	}
//}
function getElevation(){
	baseurl = 'http://maps.googleapis.com/maps/api/elevation/xml?';
	locationstr = 'path='+loc1.y+','+loc1.x+'|'+loc2.y+','+loc1.x+'&';
	samplestr = 'samples='+samp+'&';
	tailstr = 'sensor=false';
	allurl = baseurl+locationstr+samplestr+tailstr;
	//new Ajax.Request(
	//			allurl,
	//		{			
	//			method: "post",
	...
	if(window.ActiveXObject){ // If IE
		...
		xdr.onload = function(){
			...
			if (status=='OK'){
	    		...
	    		for (var i = 0; i < resultTag; i++){
					var elevMeters = resultTag[i].getElementsByTagName("elevation");
		 			ekm.push(elevMeters/1000);  
				}
			}
		}
		xdr.send(null);
