...
function loadMap(defaultPoint1, defaultPoint2) {
	if (GBrowserIsCompatible()) {
		...
		document.getElementById("latlonboundaries").innerHTML = new String('Default map boundaries for ' + volcanoname + ':<br/>Longitude=[' + minlon + ',' + maxlon + ']<br/>Latitude=[' + minlat + ',' + maxlat + ']');

		date1Str = new String(date1.getFullYear()) +
                  pad2(1 + date1.getMonth()) +
                  pad2(date1.getDate()) +
                  pad2(date1.getHours()) +
                  pad2(date1.getMinutes()) +
                  pad2(date1.getSeconds());
        	valveserverurl = new String("http://avosouth.wr.usgs.gov/valve3.4/valve3.jsp?a=plot&o=png&tz=GMT&w=900&h=300&n=1&x.0=75&y.0=20&w.0=750&h.0=240&mh.0=900&chCnt.0=1&src.0=avo_seismic_hypocenters" + 
                "&rmk.0=&rk.0=2" + 
                "&minNPhases.0=0&maxNPhases.0=100&rmk.0=&rk.0=2&minHerr.0=0&maxHerr.0=500&minVerr.0=0&maxVerr.0=500&minRMS.0=0&maxRMS.0=500&axesOption.0=D&colorOption.0=A&hypowidth.0=100" + 
                "&cntsBin.0=D&plotSeparately.0=false");
        	latlonurl = new String("&lg.0=true&north.0=" + maxlat + "&south.0=" + minlat + "&east.0=" + maxlon + "&west.0=" + minlon);
        	timeurl = new String("&st.0=" + date1Str + "000&et.0=N");
        	cumcountsurl = new String(valveserverurl + latlonurl + timeurl + "&plotType.0=cnts&minMag.0=-2&maxMag.0=10&minDepth.0=-5&maxDepth.0=20&cntsAxis.0=C");
        	timedepthurl = new String(valveserverurl + latlonurl + timeurl + "&plotType.0=map&minMag.0=-2&maxMag.0=10&minDepth.0=-5&maxDepth.0=20&cntsAxis.0=D");
        	document.getElementById('cumcounts').src = cumcountsurl;
                document.getElementById('timedepth').src = timedepthurl;
	}else{
...
function parseEqs2(ajax){
	...
	londiff = se.lng() - sw.lng();
	latdiff = nw.lat() - sw.lat();
	minlon = sw.lng().toFixed(3);
	maxlon = se.lng().toFixed(3);
	minlat = se.lat().toFixed(3);
	maxlat = ne.lat().toFixed(3);
	document.getElementById("latlonboundaries").innerHTML = new String('Default map boundaries for ' + volcanoname + ':<br/>Longitude=[' + minlon + ',' + maxlon + ']<br/>Latitude=[' + minlat + ',' + maxlat + ']');
...
Eq = Class.create({
	...
	getHtml: function(){
		...
		(!(volcanoname.match(/\b[A-Z|_]{2,}\b/))) ) {
			html += "<li><a href=\"http://giseis.alaska.edu/AVO/avoseis/TreMoR/html/sgram10min.php?subnet=" + volcanoname + "&year=" + this.yr + "&month=" + this.mon + "&day=" + this.day + "&hour=" + this.hr + "&minute=" + this.min + "&second=" + this.sec + "\" target=\"spectrogram\">Spectrogram</a></li>";
		}
		html += "</ul>";
		return html; 
	}
